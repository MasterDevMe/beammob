...
class FileUpload
{
	...
	{
		$this->upload = isset($upload) || is_array($upload) ? $upload : null;
		$this->server = $server;
	...
	public function processAll()
	{
		...
		if ($upload && is_array($upload) && !isset($upload['tmp_name'])) {
			foreach ($upload as $index => $file) {
	...
	public function validateAll() {        
		...
		// Mutiple uploads
		if ($upload && is_array($upload) && !isset($upload['name']) ) {
			foreach ($upload as $index => $f) {
				if (empty($f['tmp_name'])) {
					// Discard empty uploads
					continue;
				}
				...
				$name = $f['name'];
				$size = $size ? $size : $f['size'];
				
				$this->fileContainer = $file = new File($f['tmp_name'], $name);
				$file->name = $this->getFilename($name, $f['type'], $index, $content_range, $f['tmp_name']);
				$file->size = $this->fixIntegerOverflow(intval($size));

				if ($file->name) {
					$result = $this->validate($f['tmp_name'], $file, $f['error'], $index);
					
					// Stop when file's invalid
					if( !$result ) {
						break;
					}
				}
			...
			$name = $upload['name'];
			$size = $size ? $size : $upload['size'];
			
			$this->fileContainer = $file = new File($upload['tmp_name'], $name);
			$file->name = $this->getFilename($name, $type, 0, $content_range, $upload['tmp_name']);
			$file->size = $this->fixIntegerOverflow(intval($size));

			if ($file->name) {
				$result = $this->validate($upload['tmp_name'], $file, $upload['error'], $index);
			}
		}
